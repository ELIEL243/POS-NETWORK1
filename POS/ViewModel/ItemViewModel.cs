    ...
    public class ItemViewModel: INotifyPropertyChanged
    {
        ...
        private void EditItem()
        {
            if (item != null)
            {
                ...
                {
                    if (!string.IsNullOrEmpty(item.Description) && float.Parse(price) > 0 && selected_unit != null && !string.IsNullOrEmpty(item.Marque) && selected_category != null && int.Parse(qts) >= 0)
                    {
                        item.Unit = selected_unit.Name;
                        item.Category = selected_category.Name;
                        item.Price = float.Parse(price);
                        item.Qts = int.Parse(qts);
        ...
        private void AddItem()
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(item.Description) && itemDataEntry.validate == true && float.Parse(price) > 0 && selected_unit != null && !string.IsNullOrEmpty(item.Marque) && selected_category != null && int.Parse(qts) >= 0)
                {
                    item.Unit = selected_unit.Name;
                    item.Category = selected_category.Name;
                    item.Price = float.Parse(price);
